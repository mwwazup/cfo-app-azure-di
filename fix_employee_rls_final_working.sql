...
-- Creating in public schema since we don't have permission for auth schema
...
CREATE POLICY "select_own_pay_periods"
  ...
  USING (
    EXISTS (
      ...
      AND employee_info.user_id = public.get_clerk_user_id()
    )
  );
...
CREATE POLICY "select_own_daily_records"
  ...
  USING (
    EXISTS (
      ...
      AND ei.user_id = public.get_clerk_user_id()
    )
  );
...
SELECT public.get_clerk_user_id() AS current_clerk_user_id;

SELECT '✅ Employee LER RLS policies created successfully!' AS status;
