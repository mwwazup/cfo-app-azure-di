...
-- We need to drop every possible policy name that might exist
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'revenue_kpis') THEN
        ...
        -- Using different naming patterns that might exist
        ...
        -- Drop any other policies by querying the system
        DO $$
        DECLARE
        ...
        BEGIN
            ...
            LOOP
                EXECUTE format('DROP POLICY IF EXISTS %I ON revenue_kpis', policy_record.policyname);
